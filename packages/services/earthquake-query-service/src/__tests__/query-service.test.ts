	...
	describe("executeQuery pagination", () => {
		...
		describe("Inner Loop: Multiple Queries to Same Bucket", () => {
			it("should query same bucket multiple times until exhausted when page not full", async () => {
				...
				queryDayBucket
					...
					.mockResolvedValueOnce({
						...
						lastEvaluatedKey: {
							...
							sk: "METADATA",
							gsi1pk: "DAY#20251020",
							gsi1sk: 1760956500000,
						},
					})
					.mockResolvedValueOnce({
						items: [createMockEvent("evt5", 1760956400000, "20251020")],
				...
				expect(calls[1][0].exclusiveStartKey).toEqual({
					pk: "EVENT#evt2",
					sk: "METADATA",
					gsi1pk: "DAY#20251020",
					gsi1sk: 1760956700000,
				...
				expect(calls[2][0].exclusiveStartKey).toEqual({
					pk: "EVENT#evt4",
					sk: "METADATA",
					gsi1pk: "DAY#20251020",
					gsi1sk: 1760956500000,
				...
				expect(
					calls.slice(0, 3).every((call) => call[0].dayBucket === "20251020"),
				).toBe(true);
			});
		...
		describe("Edge Case: Bucket Iteration Order (Newest→Oldest)", () => {
			...
			it("should return events with timestamps in descending order across buckets", async () => {
				...
				expect(timestamps).toEqual([
					1760956800000, 1760870400000, 1760784000000,
				]);
